// @ts-nocheck
//TODO fix this whole file once plantviewedit is settled. seedling is still in baby stages and needs a lot of rework
...
interface ISeedlingViewEditProps extends RouteComponentPropsCustom {
  seedling: ISeedlingSchema;
}

interface ISeedlingViewEditState {
...
class SeedlingViewEdit extends Component<
  ISeedlingViewEditProps,
  ISeedlingViewEditState
> {
  // @ts-ignore
  constructor(props) {
    super(props);
  ...
  updateSeedling(type) {
    ...
    if (!type || !newSeedlingData || JSON.stringify(newSeedlingData) === "{}") {
      ...
      if (data) {
        updateOrEditItem("seedling", type, props.id, data, oldPlantData);
  ...
  render() {
    ...
    const { swipeViewIndex, activeModalId, newData } = this.state;

    const fertilizerContent = lastFertilizerUsed(seedling.fertilizerTracker);
    const soilCompLastChecked = lastChecked(seedling.soilCompositionTracker);
    const soilPh = getLastSoilPh(seedling.soilCompositionTracker);
    const soilMoisture = getLastSoilMoisture(seedling.soilCompositionTracker);
    const pestLastChecked = lastChecked(seedling.pestTracker);
    const pestName = getLastPestName(seedling.pestTracker);
    const pestTreatment = getLastPestTreatment(seedling.pestTracker);
...
export default withTracker((props: any) => {
  ...
  return {
    ...
    categories,
    editingType,
  } as ISeedlingViewEditProps;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
})(SeedlingViewEdit) as ComponentClass<ISeedlingViewEditProps, any>;
