...
import { WaterEditPro, WaterModals, WaterReadPro } from "@component";
...
interface IWaterSwipePanelProProps {
  exitEditMode: () => void;
  id: string;
...
const WaterSwipePanelPro = (props: IWaterSwipePanelProProps) => {
  ...
  const updatePlant = (type: TrackerEditingType | ModalId) => {
    ...
    if (isNull(newPlantData)) {
      ...
      return;
    } else if (type === ModalId.WATER_TRACKER) {
      // @ts-ignore
      data = {
  ...
  return (
    <div className="PlantSeedlingViewEdit">
      ...
      {editing ? (
        ...
        <WaterReadPro water={water} waterStats={waterStats} />
      )}
...
export default withTracker((props: any) => {
  ...
  return {
    ...
    waterStats,
    editing,
  } as IWaterSwipePanelProProps;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
})(WaterSwipePanelPro) as ComponentClass<IWaterSwipePanelProProps, any>;
